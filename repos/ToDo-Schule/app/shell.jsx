...
function Sidebar({activeTeam, setActiveTeam, open, onClose, onNewTask}){
  const navItems = [
  ...
  return h("aside",{className:`sidebar ${open?"open":""}`, id:"sidebar"},
    ...
    h("div",{className:"sb-scroll"},
      ...
      navItems.map(it => h("button",{
        key:it.id, className:`navitem ${activeTeam===it.id?"on":""}`,
      ...
      h("div",{className:"sb-label"},
        ...
        h("button",{title:"Bereich erstellen"},h(Icon,{n:"plus",size:13}))
      ),

      TEAMS.filter(t=>t.id!==0).map(team => {
        const cnt = TASKS.filter(t=>t.teamId===team.id&&t.status!=="done").length;
        return h("button",{
          key:team.id, className:`navitem ${activeTeam===team.id?"on":""}`,
...
function Topbar({activeTeam, view, setView, notifCount, onBell, onMenuOpen, searchVal, setSearchVal, presenceUsers}){
  const team = typeof activeTeam === "number"
    ? TEAMS.find(t=>t.id===activeTeam)
    : null;

  const crumb = team ? [team.name] : {all:"Alle Aufgaben",mine:"Mir zugewiesen",today:"Heute fällig",done:"Erledigt"}[activeTeam];
